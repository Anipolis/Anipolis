-- ================================================================
-- 085_anime_exchange_subjective_tags.sql
-- Add subjective tags to anime trade entries.
-- ================================================================

ALTER TABLE public.anime_exchange_entries
ADD COLUMN IF NOT EXISTS subjective_tags text[] NOT NULL DEFAULT '{}';
...
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM pg_constraint
        WHERE conname = 'anime_exchange_entries_subjective_tags_valid'
          AND conrelid = 'public.anime_exchange_entries'::regclass
    ) THEN
        ALTER TABLE public.anime_exchange_entries
        ADD CONSTRAINT anime_exchange_entries_subjective_tags_valid
        CHECK (
            cardinality(subjective_tags) <= 3
            AND subjective_tags <@ ARRAY[
                '泣ける',
                '心温まる',
                '胸熱',
                '燃える',
...
DROP FUNCTION IF EXISTS public.create_anime_exchange(bigint, text);
DROP FUNCTION IF EXISTS public.create_anime_exchange(bigint, text, text[]);
