<script lang="ts">
    ...
    async function handleFileChange(e: Event) {
        ...
        if (!file) { imageFile = null; imagePreview = null; resizedBlob = null; return; }
        imageFile = file;
...
<div class="register-section">
    ...
    <form method="POST" action="?/registerAnime" use:enhance={async ({ formData }) => {
        ...
        <div class="form-row">
            ...
            <div class="form-group form-group--wide">
                ...
                <label class="cover-upload-label" class:has-preview={!!imagePreview}>
                    ...
                    {:else}
                        <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" aria-hidden="true"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>
                        <span class="cover-upload-hint">{imageProcessing ? '処理中...' : '画像をアップロード（JPEG/PNG/WebP）'}</span>
                    {/if}
                    <input type="file" accept="image/jpeg,image/png,image/webp" onchange={handleFileChange} disabled={imageProcessing} class="cover-file-input" />
                </label>
                {#if !imageFile}
                    <input id="rf-cover" name="cover_url" type="url" class="rf-input" style="margin-top:6px" placeholder="または画像 URL を直接入力..." />
                {/if}
            </div>
        </div>
...
<style>
    .register-section { max-width: 760px; }
    .register-title {
        font-size: 1.1rem;
        font-weight: 700;
        margin-bottom: 20px;
    ...
    .form-success a { color: var(--status-watching); font-weight: 600; }
    .form-error {
        padding: 12px 16px;
        border-radius: 8px;
        background: #dc262618;
    ...
    .form-group { display: flex; flex-direction: column; gap: 5px; flex: 1 1 180px; }
    ...
    .cancel-link { font-size: 0.85rem; color: var(--text-muted); text-decoration: none; }
    ...
    .cover-upload-label:hover { border-color: var(--accent); background: var(--hover-bg); }
    .cover-upload-label.has-preview { height: 160px; border-style: solid; padding: 0; }
    .cover-preview { width: 100%; height: 100%; object-fit: contain; object-position: center; }
    .cover-change-hint {
        position: absolute;
    ...
    .cover-upload-hint { text-align: center; padding: 0 8px; line-height: 1.4; }
</style>
