...
import type { Anime, AnimeStatus, Event, Notification, Post, UserAnimeEntry } from "$lib/types";
...
export async function getAnimeList(
	...
	// eslint-disable-next-line @typescript-eslint/no-explicit-any
	let query: any = supabase.from("anime").select("*").order("created_at", { ascending: false }).limit(limit);
...
export async function getAnime(
	...
	const { data, error } = await supabase.from("anime").select("*").eq("id", animeId).maybeSingle();
	...
	const [popularityRes, trendingRes, topRatedRes] = await Promise.all([
		...
		// eslint-disable-next-line @typescript-eslint/no-explicit-any
		(supabase.from("anime_top_rated") as any)
...
export async function getAnimeRankingTrending(supabase: SupabaseClient<Database>, limit = 20): Promise<Anime[]> {
	...
	const animes = await fetchAnimesByIds(
		supabase,
		rd2.map((r) => r.anime_id as string),
	);
...
export async function getAnimeRankingTopRated(supabase: SupabaseClient<Database>, limit = 20): Promise<Anime[]> {
	...
	const animes = await fetchAnimesByIds(
		supabase,
		rd3.map((r) => r.anime_id as string),
	);
	return animes.map((a) => ({
		...a,
