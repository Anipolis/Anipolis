...
const activeMode = $derived((form?.mode ?? data.mode) as "login" | "register");
const next = $derived(data.next ?? "/");
</script>

<svelte:head> <title>{activeMode === 'register' ? 'アカウント作成' : 'ログイン'} - Anipolis</title> </svelte:head>

<div class="page-container auth-page">
	<main class="auth-panel">
		<div class="settings-card">
			<div class="settings-header-row">
				<h1 class="settings-title">{activeMode === 'register' ? 'アカウント作成' : 'ログイン'}</h1>
			...
			<div class="auth-tabs" aria-label="認証メニュー">
				<a href="/auth?mode=login&next={encodeURIComponent(next)}" class:active={activeMode === 'login'}
					>ログイン</a
				>
				<a href="/auth?mode=register&next={encodeURIComponent(next)}" class:active={activeMode === 'register'}
					>アカウント作成</a
				>
			</div>
