...
import { checkIsFollowing, enrichPostsWithCounts, getFollowCounts, getUserAnimeList } from "$lib/server/queries";
...
export const load: PageServerLoad = async ({ params, locals: { supabase, safeGetSession } }) => {
	...
	const [rawPostsResult, followCounts, isFollowing, trendingResult, animeList] = await Promise.all([
		...
		user && user.id !== profile.id ? checkIsFollowing(supabase, user.id, profile.id) : Promise.resolve(false),
