<script lang="ts">
    ...
    const endMs = $derived(
        data.event.duration_minutes
            ? scheduledMs + data.event.duration_minutes * 60 * 1000
            : null,
    );
    ...
    const timerLabel = $derived.by<string>(() => {
        switch (status) {
            ...
            case 'upcoming':  return `開始まで ${formatHMS(scheduledMs - now)}`;
            case 'live':      return `経過 ${formatHMS(now - scheduledMs)}`;
            case 'ended':     return `終了（${data.event.duration_minutes}分間）`;
        }
    });
    ...
    const contentWithTag = $derived(
        postContent.includes(`#${data.event.hashtag}`)
            ? postContent
            : postContent + hashtagSuffix,
    );
...
<svelte:head>
    <title>{data.event.title} — Anipolis</title>
</svelte:head>
...
<div class="page-container">
            ...
            <div class="event-room-top">
                <div>
                    <div class="event-room-hashtag">
                        <a href="/hashtag/{data.event.hashtag}" class="hashtag-link">
                            #{data.event.hashtag}
                        </a>
                    </div>
                ...
                <form method="POST" action="?/createPost" use:enhance>
                    ...
                    <div class="composer-body">
                        ...
                        <div class="composer-footer">
                            ...
                            <button type="submit" class="btn btn-primary btn-sm" disabled={overLimit || !postContent.trim()}>
                                投稿
                            </button>
                        </div>
                    </div>
                    <p class="composer-hint">
                        投稿に <strong>#{data.event.hashtag}</strong> が自動で付与されます
                    </p>
                </form>
            </div>
        {:else if !data.user}
    ...
    <aside class="sidebar-column">
        <div style="margin-bottom:16px;">
            <a href="/calendar" class="btn btn-ghost" style="width:100%;">
                ← カレンダーへ戻る
            </a>
        </div>
