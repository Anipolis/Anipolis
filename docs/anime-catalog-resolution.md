...
3. 検証済み日本語タイトル、またはODbLに保存済みの未検証日本語同義語候補と、しょぼいタイトルが正規化完全一致し、
   対象シーズン内で双方から一意になる対応

曖昧な対応は推測せず未解決一覧へ出す。Wikidata対応だけでは、分割クールなどで一つのTIDが複数のMAL作品を表す場合が
あるため、WikidataだけでTID対応が得られても、しょぼいカレンダーのタイトルを表示タイトルに使うのは既存の日本語タイトル
照合候補とも正規化完全一致する場合に限る。
ここでの正規化はNFKCに加え、空白、波ダッシュ、ダッシュ、末尾の明示的な期・Season表記だけを統一する。
部分一致や編集距離による類似検索は自動確定に使わない。ODbLの未検証同義語候補も単独では公開根拠にせず、
しょぼいの対象シーズンタイトルと一意に一致した時点で初めて表示タイトルの検証根拠にする。
